...
const Payment = () => {
  ...
  const handlePay = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('https://diplomreact.apt142.ru/orders', {
        cart: cart,       
        delivery: form    
      }, { 
                  ...
                  <div key={item.id} className="flex justify-between items-center text-sm p-3 rounded-xl bg-[var(--bg-color)]/50 border border-[var(--glass-border)]">
                    <div className="flex items-center gap-3 overflow-hidden">
                       <div className="w-10 h-10 rounded-lg bg-white p-1 shrink-0">
                          <img src={item.image.startsWith('http') ? item.image : `https://diplomreact.apt142.ru/${item.image}`} className="w-full h-full object-contain" />
                       </div>
