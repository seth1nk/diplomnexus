...
const Dashboard = ({ user }) => {
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-[var(--bg-color)]">
       ...
       <span className="text-[var(--accent-color)] font-mono animate-pulse uppercase tracking-widest">System Booting...</span>
    </div>
  );
    ...
    <div className="pt-28 pb-20 max-w-[1900px] mx-auto px-4 min-h-screen">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-end border-b border-[var(--glass-border)] pb-8 mb-10 gap-6">
        <div>
          ...
          <p className="text-[var(--text-color)] opacity-60 font-mono text-sm uppercase">:: OPERATOR: {user?.name} :: STATUS: ONLINE</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_400px] gap-10 items-start">
        ...
        <div className="w-full">
            ...
            <AnimatePresence mode='wait'>
              {currentItems.map((item) => (
                <motion.div 
                  ...
                  className="uiverse-card group h-[420px] w-full border border-[var(--glass-border)] hover:border-[var(--accent-color)]/40 transition-all rounded-3xl overflow-hidden bg-[var(--card-bg)]"
                >
                  <div className="p-5 flex flex-col h-full justify-between">
                    <div className="relative h-52 w-full rounded-2xl overflow-hidden border border-[var(--glass-border)] bg-white p-4">
                      <img 
                    ...
                    <div className="mt-4">
                       <h3 className="font-black text-lg text-[var(--text-color)] uppercase leading-tight truncate mb-2">{item.name}</h3>
                       <p className="text-xs text-[var(--text-color)] opacity-50 line-clamp-2">{item.description}</p>
                    </div>

                    <button 
                      onClick={() => addToCart(item)}
                      className="mt-4 w-full py-3.5 rounded-xl bg-[var(--accent-color)]/10 text-[var(--accent-color)] font-bold text-[10px] tracking-[0.2em] border border-[var(--accent-color)]/20 hover:bg-[var(--accent-color)] hover:text-black transition-all flex items-center justify-center gap-2 active:scale-95"
                    >
          ...
          {totalPages > 1 && (
            <div className="flex justify-center gap-2 mt-10">
              <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-3 glass rounded-xl hover:text-[var(--accent-color)] disabled:opacity-20"><ArrowLeft/></button>
              <div className="flex gap-2">
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i} onClick={() => setCurrentPage(i + 1)} className={`w-12 h-12 rounded-xl font-bold font-mono transition-all ${currentPage === i + 1 ? 'bg-[var(--accent-color)] text-black' : 'glass opacity-50'}`}>{i + 1}</button>
                ))}
              </div>
              <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-3 glass rounded-xl hover:text-[var(--accent-color)] disabled:opacity-20"><ArrowRight/></button>
            </div>
          )}
        </div>
        ...
        <aside className="w-full">
          <div className="glass p-6 rounded-[2.5rem] sticky top-28 border border-[var(--glass-border)] shadow-2xl">
            ...
            <div className="flex flex-col gap-3 mb-6 max-h-[450px] overflow-y-auto pr-1 custom-scroll">
              {cart.length === 0 ? (
                <div className="text-center py-20 opacity-20 flex flex-col items-center border-2 border-dashed border-[var(--glass-border)] rounded-3xl">
            ...
            <div className="border-t border-[var(--glass-border)] pt-5">
              <div className="flex justify-between mb-6 items-end">
                <span className="text-[var(--text-color)] opacity-40 font-bold text-[10px] uppercase tracking-widest">Итого к оплате</span>
      ...
      {orders.length > 0 && (
        <div className="mt-20 border-t border-[var(--glass-border)] pt-12" onMouseEnter={() => setIsPaused(true)} onMouseLeave={() => setIsPaused(false)}>
          <div className="flex justify-between items-center mb-8 px-4">
            ...
            <div className="flex gap-2">
              <button onClick={prevOrder} className="p-3 rounded-xl glass hover:text-[var(--accent-color)]"><ChevronLeft size={24} /></button>
              <button onClick={nextOrder} className="p-3 rounded-xl glass hover:text-[var(--accent-color)]"><ChevronRight size={24} /></button>
            </div>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 gap-6 overflow-hidden">
             <AnimatePresence mode='popLayout'>
               {visibleOrders.map((order, i) => (
                 <motion.div 
                   key={`${order.id}-${orderIndex}-${i}`} 
                   initial={{ x: 100, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -100, opacity: 0 }}
                   className="glass p-6 rounded-[2rem] border border-[var(--glass-border)] relative flex flex-col justify-between min-h-[200px] bg-[var(--card-bg)]/50"
                 >
                    ...
                    <p className="text-xs text-[var(--text-color)] opacity-70 font-bold line-clamp-2 mb-4">{order.content}</p>
